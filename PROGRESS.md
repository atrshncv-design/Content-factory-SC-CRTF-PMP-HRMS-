...
| 45 | LIVE-фикс сценария (16.08, жалоба «сценарий не сделали») | ✅ done: hermes CLI в -Q без --reasoning none отдаёт в stdout ТОЛЬКО reasoning (49КБ) — Exp Parse хватал первую пару <SCRIPT> из рассуждений → garbage. Фикс: --reasoning none для всех скиллов bridge + Exp Parse берёт ПОСЛЕДНЮЮ пару <SCRIPT> и разворачивает JSON (full_text/hook+body+cta). Верифицировано: bridge-ответ чистый (1 пара, 556 байт), sim Exp Parse ok:true, live POST script_only → {ok:true}, 0 кредитов |
| 46 | LIVE-фикс меток сценария (16.08, «Хук:/Основная часть:/Вывод и призыв:» в тексте) | ✅ done: вычистка служебных меток (RU+EN) в cleanText/clean — 2 копии в shorts (Exp Parse + Code Build payload) и 4 в wf-tg-bot (AU/DU Parse, submit body). Sim + live script_only: метки вырезаны, ok:true, 0 кредитов. Деплой: import+publish shorts+tg-bot, restart, 23 активны, tg-trigger 403 |
| 47 | LIVE-фикс кнопок и доставки видео (16.08, «нажал принять — ничего не сработало» / «видео не пришло») | ✅ done, 3 бага: (1) SH Format async ссылался на $('SH Topic') — ноду, которой нет в callback-прогоне (topic теперь из quick_payload); (2) INSERT generations без script_id (NOT NULL) — добавлен script_id=0 + новый узел SH HTTP gen link (сессия ← generation_id); (3) вебхук creatify: IF auth требовал x-factory-secret, а creatify шлёт Authorization: Bearer = CREATIFY_API_KEY → 403. Фикс: Bearer==CREATIFY_API_KEY || x-factory-secret==секрет (fail-closed) + фолбэк поиска сессии по generation_id. Видео пользователя доставлено реплеем колбэка (gen 18, done). Sim: SH Format async (INSERT+фолбэк) ✓, вебхук-lookup по generation_id ✓ (реальные данные), pytest 25/25 |
| 48 | LIVE-фикс доставки больших видео (16.08, «видео не пришло в чат» — ролик 39 МБ) | ✅ done: Telegram Bot API при sendVideo по URL отклоняет файлы >20 МБ («wrong type of the web page content»), а n8n шлёт URL → 400. Фикс: новый узел HTTP download video (responseFormat file → binary "data") + Telegram stage3/stage3 auto переведены на binaryData=true/binaryPropertyName=data (chatId/caption — кросс-ссылки на Build stage3), IF auto approve — кросс-ссылка. Верифицировано live: exec 5196 success, sendVideo message 229 с кнопками; pytest 25/25 |
| 49 | LIVE-фикс длительности роликов ≤30 сек (16.08, жалоба «40 сек = 10 кредитов вместо 5») | ✅ done, корень: POST /api/ai_shorts/ НЕ принимает video_length (проверено по официальному OpenAPI docs.creatify.ai) — длительность = длине скрипта (замер: 86 слов = 40 сек = 10 кредитов, темп 129 сл/мин). Фикс 3 слоя: (1) скилл scriptwriter: 90–110 → 55–62 слова; (2) Exp Build prompt (shorts): 90–110 → 55–60; (3) Code Build payload: жёсткая обрезка до 60 слов по границе предложения (видео_length 15/30/45/60, дефолт 30); wf-tg-bot CT/AU/AU RG: words = dur*60/30. Верифицировано: sim 86→48 слов (~22 сек), live script_only → {ok, 48 слов}, pytest 25/25, валидатор 0 issues |
| 50 | LIVE-фикс URL→видео (16.08, жалоба «сценарий не пришёл даже») | ⚠️ v1 (0-сентинель) НЕ сработала: db-bridge (node:sqlite) ВКЛЮЧАЕТ foreign_keys (в отличие от sqlite3 CLI, PRAGMA=0) → scripts.topic_id=0 → FOREIGN KEY constraint failed (exec 5235). ✅ v2 (задеплоена): новые узлы DU Build topic + DU HTTP topic (INSERT INTO topics: title/source_url=URL) + DU Build script берёт lastInsertRowid темы; GE/SC откат к исходному (темы гарантированы потоком). SQL обеих нод проверены через bridge (insert+cleanup), import+реактивация+restart, active=1, сессия пользователя сброшена из QUICK_URL_GENERATING в IDLE, webhook pending=0. Урок: FK enforced в bridge — сентинель 0 допустим только для колонок без REFERENCES. Ждёт: повторный платный прогон пользователя |
| 51 | LIVE-фикс «Не удалось запустить генерацию: пустой ответ» (16.08, URL→видео, exec 5249) | ✅ done: HTTP POST real (wf-creatify-submit) падал на «JSON Body не валидный JSON» — jsonBody был в legacy-синтаксисе `{{ JSON.stringify(...) }}` без префикса `=`, который n8n 2.x в jsonBody НЕ вычисляет (до parseJsonParameter дошёл литерал `{{` → «Expected property name at position 1»). Фикс: `={{ Object.assign({}, ...) }}` (объект, как во всех рабочих jsonBody; JSON.stringify убран). Единственное вхождение паттерна `{{`-без-`=` по всем воркфлоу (греп). Выражение проверено в Node (валидный JSON с webhook_url); `={{ }}`-паттерн доказан в проде (DU HTTP submit). Кредиты не списаны (падение до запроса); generation 19 (pending, без creatify_id) помечен failed (поллер не подхватит). Деплой: import+реактивация+restart, active=1. Ждёт: повторный платный прогон пользователя (~5 кред за 30 сек) |
| 52 | LIVE-фикс «пустой ответ» v2 — creatify 400: невалидные script_style/visual_style (16.08, exec 5263) | ✅ done, корень: creatify вернул `400 {"script_style": ["informative is not a valid choice"], "visual_style": ["default is not a valid choice"]}` — DU Build submit хардкодил несуществующие стили (кредиты не списаны — 400 до генерации). Ответ на вопрос пользователя: да, link_to_videos ждёт `link` (UUID из POST /api/links/ по URL) — шлём правильно; сценарий опционален (override_script). Фикс: `script_style: 'DiscoveryWriter'` (дефолт API) + `visual_style: 'DynamicProductTemplate'` (подходит для 9x16 и страницы товара) — оба из официального OpenAPI docs.creatify.ai (enum'ы вытащены из post-apilink_to_videos.md). Скилл json-builder уже учит валидным стилям (путь AU не затронут). Деплой: import+реактивация+restart, active=1, webhook pending=0. Ждёт: повторный платный прогон пользователя |
| 53 | LIVE-фикс «кредиты списали, а ролик не отправили» (16.08, gen 21/22) | ✅ done, корень: SUCCESS-ветка Switch DU submit была ПУСТОЙ — после submit флоу не привязывал сессию к генерации → вебхук: «Сессия не привязана к генерации #N (нет session-link)» (Build stage3, tg_user_id NULL) — видео скачано, но не отправлено. Кредиты списаны дважды (21: 04e1945f, 22: ad1776b7). Фикс v3: 4 новых узла в success-ветку — DU Build gen link + DU HTTP gen link (UPDATE sessions SET generation_id, script_id COALESCE) + DU Format ok («✅ Генерация запущена») + TG du ok. Деплой: import+реактивация+restart, active=1. Доставка зависших: привязка сессии → reset status=running → реплей колбэка (exec 5332/5333 Respond done, 17 нод) — ОБА видео доставлены в чат (gen 21 4290be…, gen 22 93ee6e…). Урок: пустой выход Switch = молчаливая потеря доставки; все submit-пути должны связывать сессию |
|| 54 | LIVE-фикс цены URL→видео (16.08, «почему 15.5–21 кред вместо 5?») | ✅ done, корень: в payload шёл `model_version: 'aurora_v1_fast'` (Aurora Fast = 0.5 кред/СЕК по офиц. billing docs.creatify.ai), а «URL to Video = 5 кред/30 сек» — тариф СТАНДАРТНОЙ модели. Спеку ввели в заблуждение пометкой «aurora_v1_fast (дешевле)» — на деле дороже для ≥10 сек (30с: 15 vs 5). Фикс: model_version → standard/удалён во ВСЕХ местах: DU Build submit (payload), AS/AU Build prompt (промпты), json-builder.md (скилл: тарифы исправлены), спеки 03/04 (пометки). Факт по API: gen 21 (30с) = 15.5, gen 22 (41с) = 21 — после фикса будет 5 кред/30с + 1 кред за создание link. Деплой: wf-tg-bot import+реактивация+restart, скилл на сервер. Ждёт: платный прогон для подтверждения цены |

## Фича: Аудит и починка TG-кнопок (17.08.2026, autopilot) — В РАБОТЕ
Спека: `.scratch/audit-tg-buttons/spec.md` · Тикеты: `.scratch/audit-tg-buttons/issues/` (01–05)

| # | Тикет | Статус |
|---|-------|--------|
| 01 | Автотест покрытия кнопок | ✅ done (тест падает, 123 нарушения — baseline) |
| 02 | Починить expression-клавиатуры в сценарии «Видео с аватаром» | ✅ done |
| 03 | Починить кнопки выбора длительности | ✅ done |
| 04 | Добавить недостающие кнопки Меню/Отмена | ✅ done |
| 05 | Бэкап, валидация и отчёт | ✅ done |
