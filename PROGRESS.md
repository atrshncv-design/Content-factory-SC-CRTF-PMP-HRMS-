...
| 50 | LIVE-фикс URL→видео (16.08, жалоба «сценарий не пришёл даже») | ✅ done, корень: exec 5208 — DU Build script вставлял scripts с topic_id=NULL при NOT NULL constraint (соседи того же класса: GE Build insert topic_id из sessions, SC Cont build || null). Фикс: 0-сентинель (прецедент script_id=0; FK off подтверждён PRAGMA=0, SQL-тест insert+cleanup на сервере). Деплой: import + реактивация (урок: import DEACTIVATES — update:workflow --active=true + restart), active=1, webhook pending=0. Меню-ошибка exec 5201 — транзиентный таймаут Telegram API (не баг). Ждёт: повторный платный прогон пользователя |
