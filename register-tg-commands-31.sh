#!/usr/bin/env bash
# =============================================================================
# register-tg-commands-31.sh — регистрирует 31 команду контент-завода в Telegram
# боте через setMyCommands (UX-реворк 14.08: menu, instruction, url2video).
# Копия register-tg-commands.sh с tg-commands-31.json и verify на 31.
...
PAYLOAD="$FACTORY_DIR/tg-commands-31.json"
...
import sys, json
want = {"start","menu","instruction","help","status","mode","onboard","start_cycle","url2video","shorts","cancel","topics","competitors","accounts","budget","client","clients","reload_skills","ping","creators","creator","creator_content","audience","transcript","comments","upload_avatar","my_avatars","asset","product","banner","publish_type"}
...
if [ "$registered" = "1" ]; then
  log "OK: all 31 factory commands registered (scopes: default, all_private_chats, all_group_chats)"
  exit 0
fi
log "WARNING: not stable after 5 attempts. Re-run: bash $FACTORY_DIR/register-tg-commands-31.sh" >&2
exit 0
