...
## 21a. Публикация: контракты 7 платформ (тикет 06, 16.08) — DONE (до точки списания)
- **Маршрутизация upload — Switch upload needed (3 ветки, не перевёрнут)**: out0 — file_ids целочисленные (медиа уже загружено) → skip; out1 — text-only (file_ids пуст И нет видео-платформ instagram/youtube/tiktok) → skip; out2 (extra) — upload нужен (видео-платформы без file_ids ИЛИ file_ids = URL-строка от бота). Питфолл закрыт: text-only больше НЕ уходит в upload/init (ранее: 422 «Не удалось загрузить файл по ссылке» в real / фейковый file_id 67890 в mock).
- **HTTP real upload**: url = file_ids[0], если это http(s) URL от бота (CP/AU Build publish body шлют video_output_url), иначе фолбэк `$env.WEBHOOK_URL + 'media/' + generation_id + '.mp4'`; project_id числом; neverError вложенный — вебхук всегда отвечает.
- **Контракт 7 платформ (Code build details)**:
  - видео: instagram/youtube/tiktok → publication_type 4 (REELS_SHORTS_CLIPS) + file_ids из upload/init; youtube_privacy_status=1 + title; instagram_share_to_feed при type 4; tiktok_comment/duet/stitch из tiktok_options;
  - текст: telegram/x/threads/vk → publication_type 1 (POST), только content;
  - account_id: account_ids[запроса] → social_accounts (БД, синк wf-sync-accounts) → legacy {ig:101, yt:102, tiktok:106, threads:104, x:105}. telegram/vk legacy-фолбэка НЕТ: до подключения аккаунтов detail уходит без account_id — Code filter accounts отрежет недоступные, публикация остальных платформ не ломается.
- **Адаптация caption — гарантии на стороне публикации (дополняют контракт caption-adapter)**:
  - telegram: markdown (*b*, _i_, [t](url)) СТРИПИТСЯ в Code build details и Code merge adapted — postmypost публикует TG plain-текстом, символы разметки иначе видны буквально;
  - x: жёсткий лимит 280 символов (обрезание + «…», итого 278) — страховка к контракту caption-adapter ≤280;
  - threads: длинный разговорный текст (контракт caption-adapter), ограничений нет;
  - vk: текст БЕЗ markdown, ссылки только raw URL, приветствие по client_profile.name (контракт caption-adapter). ⚠️ Ограничение postmypost: VK-пост уходит от подключённого сообщества/страницы; видео-посты в VK вне scope (7 платформ = IG/TikTok/YT видео; TG/X/Threads/VK текст).
- **Аккаунты (G01)**: в БД 5 соцаккаунтов (IG/YT/Threads/X/TikTok, синк 13.08); VK и TG в проекте postmypost ОТСУТСТВУЮТ — подключает пользователь перед платными тестами; публикация в VK/TG до подключения → 422 «аккаунт не подключён» (ожидаемо, F-3).
- **wf-publish-status** (cron */2): (mock)-маркер из tg-алерта убран (сообщение «Опубликовано #id» чистое); мёртвые IF any?/NoOp no rows в репо уже отсутствуют (live FIX-13); neverError на HTTP GET real / UPDATE / tg-нодах.
- **wf-sync-accounts** (cron 0 * * * *): UPSERT social_accounts через db-bridge, алерт при connection_status=2; mock-снимок = 5 аккаунтов (IG=101, YT=102, Threads=104, X=105, TikTok=106), отсутствие VK задокументировано в jsCode Code mock accounts.
- Проверено бесплатно (0 кредитов): валидатор 0 issues по всем трём воркфлоу; sim 30/30 (маршрутизация Switch ×5 сценариев, details[] text/media, TG-стрип, X≤280, NO_ACCOUNT для TG/VK, upload-URL, mock 5 аккаунтов, UPSERT, алерт без (mock)); локальная БД data/factory.db — social_accounts пуста (дев-копия), эталон состояния — серверная БД (синк 13.08).
