#!/usr/bin/env bash
# =============================================================================
# register-tg-commands.sh — регистрирует 15 команд контент-завода в Telegram
...
PAYLOAD="$FACTORY_DIR/tg-commands-15.json"
...
# --- Проверка: getMyCommands = ровно наши 15 (все имена, без чужих) ---
verify_ours() {
...
import sys, json
want = {"start","help","status","mode","onboard","start_cycle","cancel","topics","competitors","accounts","budget","client","clients","reload_skills","ping"}
...
if [ "$registered" = "1" ]; then
  log "OK: all 15 factory commands registered (scopes: default, all_private_chats, all_group_chats)"
  exit 0
fi
